...
def everything(args):
    ...
    indir = args.indir
    title = '70_30-Heuristic-Play' # A nice, short title
    ...
    # no_actions = np.array(loadjson(indir,'no-actions - '+side+'.json')) # is an nx2 with actions for only two paddles [Rgoal, Rcenter]
    ...
    for side in ['right','left']:
        scores_both.append(np.array(loadjson(indir,'scores - '+side+'.json'))) #first is right side, second is left
    ...
    scores_Ll = scores_both[1][:,0] # This is RL agent
    scores_Lr = scores_both[1][:,1]
    ...
    for eachgame in scores_Rr:
        ...
        if i%DQNint==0: # If episode is a multiple of DQNint, take the average of the previous ten episodes
            avg = (np.mean(scores_Rr[i-DQNint:i]))
            avgPts_R.append(avg)
        i+=1
    ...
    pctWins_R = totWins_R/len(scores_both[0])
    ...
    for eachgame in scores_Ll:
        ...
        if i%DQNint==0: # If episode is a multiple of DQNint, take the average of the previous ten episodes
            avg = (np.mean(scores_Ll[i-DQNint:i]))
            avgPts_L.append(avg)
        i+=1
    ...
    pctWins_L = totWins_L/len(scores_both[1])
...
if __name__=='__main__':
    ...
    parser.add_argument('--indir',default = 'shashvat_pretrained10000_70_30',type=str) # If want to load weights from a specific subdirectory... defaults to the latest training (saved in trained_weights/latest)
    parser.add_argument('--savedir',default = 'shashvat_pretrained10000_70_30',type=str) # Specify directory to save selected stats in (will save everything, including weights, to trained_weights/<name>)
