...
# def init_game(stw):
    ...
    # def foosPong_ai(states,eps,yesRender,withTFmodel,nos,id):
        # noPaddles = nos[0] # A list (ordered pair) containing the number of paddles on each team
        # noBalls = nos[1][0] # A scalar, number of balls
        # output = np.asarray(states, dtype='float32')
        # dimstat = output.shape[0]*output.shape[1] # Compute number of dimensions in state
        # output = foosPong(output.reshape((1,dimstat))
        # team_Q_values = tf.reshape(output, noPaddles)
